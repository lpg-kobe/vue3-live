<template>
  <div class="container_tiem" v-if="tab.length > 0">

    <div class="tabs right_tabs">
      <ul>
        <li v-for="(item, index) in tab" :class="{active: active === item.menuType}" @click="clickTab(item.menuType)" :key="index" v-show="item.menuType === 7"  :title="tabTransEn(item.menuType)">{{ tabTransEn(item.menuType) }}</li>
      </ul>
      <a class="fr more-link" target="_blank" :href="`http://photo.ofweek.com/wap/activity/index?id=${room.picId}`">{{ $t('common.more') }}</a>
    </div>
