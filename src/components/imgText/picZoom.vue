...
export default {
  ...
  mounted () {
    ...
    bigWrap.onmouseover = function(){//鼠标移动到box上显示大图片和选框
        bigBox.style.display = "block";
        moveMk.style.display="block";
    }
    bigWrap.onmouseout = function(){//鼠标移开box不显示大图片和选框
        bigBox.style.display = "none";
        moveMk.style.display="none";
    }
    bigWrap.onmousemove = function(e){//获取鼠标位置
        ...
        if(_top<=0)//滑到box的最顶部
            _top = 0;
        else if(_top>=bigWrap.offsetHeight-moveMk.offsetHeight)//滑到box的最底部
            _top = bigWrap.offsetHeight-moveMk.offsetHeight ;
        if(_left<=0)//滑到box的最左边
            _left=0;
        else if(_left>=bigWrap.offsetWidth-moveMk.offsetWidth)//滑到box的最右边
